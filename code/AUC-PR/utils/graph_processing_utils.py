...
def subgraph_isomorphism_edge_counts(edge_index, **kwargs):
    ...
    if not directed:
        ...
        subgraph_edges = subgraph_dict['subgraph'].get_edges().tolist()

    G_gt = gt.Graph(directed=directed)
    G_gt.add_edge_list(edge_index)
    ...
    counts = np.zeros((len(edge_index), len(subgraph_dict['orbit_partition'])))
    ...
    for sub_iso_curr in sub_iso:
        ...
        for i, edge in enumerate(subgraph_edges):
            ...
            counts[edge2index[mapped_edge], edge_orbit] += 1

    counts = counts / subgraph_dict['aut_count']

    counts = torch.tensor(counts)

    return counts, index2edge
...
def edge_automorphism_orbits(edge_list, **kwargs):
    ...
    aut_group_edges = gt_topology.subgraph_isomorphism(line_graph, line_graph, induced=False, subgraph=True,
                                                       generator=False)
    ...
    orbit_partition = {}
    for vertex, orbit in orbit_membership.items():
        orbit_partition[orbit] = [inverse_mapping[vertex]] if orbit not in orbit_partition else orbit_partition[
                                                                                                    orbit] + [
                                                                                                    inverse_mapping[
                                                                                                        vertex]]
