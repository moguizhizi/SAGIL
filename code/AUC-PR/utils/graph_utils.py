...
def deserialize(data):
    ...
    keys = ('nodes', 'r_label', 'g_label', 'n_label', 'subgraph_size', 'enc_ratio', 'num_pruned_nodes',
            'nodes_identifiers_from_subgraph',
            'edge_identifiers_from_subgraph')


    return dict(zip(keys, data_tuple))


def deserialize_edge_identifiers(data):
    data_tuple = pickle.loads(data)
    keys = ('edge_identifiers',)
    return dict(zip(keys, data_tuple))

def deserialize_node_identifiers(data):
    data_tuple = pickle.loads(data)
    keys = ('node_identifier',)
    return dict(zip(keys, data_tuple))

def deserialize_for_subgraph(data):
    data_tuple = pickle.loads(data)
    return data_tuple
