...
class RGCNBasisLayer_1(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim, num_rels, num_bases=-1, bias=None,
                 activation=None, dropout=0.0, edge_dropout=0.0, is_input_layer=False, has_attn=False,
                 num_substructure=0, edge_num_substructure=0, gsn_type=0, no_jk=False):
        super(
            RGCNBasisLayer_1,
            self).__init__(
            inp_dim,
        ...
        self.is_input_layer = is_input_layer
        self.has_attn = has_attn
        self.gsn_type = gsn_type
        self.num_substructure = num_substructure
        self.edge_num_substructure = edge_num_substructure
        ...
        if self.gsn_type == 0:
            additional = 2 * self.num_substructure
        elif self.gsn_type == 1:
            additional = self.edge_num_substructure
        elif self.gsn_type == 2:
            additional = 2 * self.num_substructure + self.edge_num_substructure
        else:
            raise ValueError('gsn_type is error, gsn_type：{}'.format(self.gsn_type))

        if self.has_attn:
            self.A = nn.Linear(2 * self.inp_dim + 2 * self.attn_rel_emb_dim + additional, inp_dim)
            self.B = nn.Linear(inp_dim, 1)
    ...
    def propagate(self, g, attn_rel_emb=None, norm=None):
        ...
        def msg_func(edges):
            ...
            if self.has_attn:
                if self.gsn_type == 0:
                    e = torch.cat(
                        [edges.src[input_], edges.dst[input_], edges.src['identifier'], edges.dst['identifier'],
                         attn_rel_emb(edges.data['type']),
                         attn_rel_emb(edges.data['label'])], dim=1)
                elif self.gsn_type == 1:
                    e = torch.cat([edges.src[input_], edges.dst[input_], edges.data['identifier'],
                                   attn_rel_emb(edges.data['type']),
                                   attn_rel_emb(edges.data['label'])], dim=1)
                elif self.gsn_type == 2:
                    e = torch.cat(
                        [edges.src[input_], edges.dst[input_], edges.src['identifier'], edges.dst['identifier'],
                         edges.data['identifier'],
                         attn_rel_emb(edges.data['type']),
                         attn_rel_emb(edges.data['label'])], dim=1)
                else:
                    raise ValueError('gsn_type is error, gsn_type：{}'.format(self.gsn_type))
...
class RGCNBasisLayer_2(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim, num_rels, num_bases=-1, bias=None,
                 activation=None, dropout=0.0, edge_dropout=0.0, is_input_layer=False, has_attn=False,
                 num_substructure=0, edge_num_substructure=0, gsn_type=0, no_jk=False):
        super(
        ...
        # add basis weights
        # self.weight = basis_weights
        self.weight = nn.Parameter(torch.Tensor(self.num_bases, self.inp_dim + self.num_substructure, self.out_dim))
        self.w_comp = nn.Parameter(torch.Tensor(self.num_rels, self.num_bases))
    ...
    def propagate(self, g, attn_rel_emb=None, norm=None):
        ...
        # generate all weights from bases
        weight = self.weight.view(self.num_bases,
                                  (self.inp_dim + self.num_substructure) * self.out_dim)
        weight = torch.matmul(self.w_comp, weight).view(
            self.num_rels, (self.inp_dim + self.num_substructure), self.out_dim)
        ...
        def msg_func(edges):
            w = weight.index_select(0, edges.data['type'])
            msg = edges.data['w'] * torch.bmm(torch.cat((edges.src[input_], edges.src['identifier']), 1).unsqueeze(1),
                                              w).squeeze(1)
            curr_emb = torch.mm(edges.dst[input_], self.self_loop_weight)  # (B, F)
