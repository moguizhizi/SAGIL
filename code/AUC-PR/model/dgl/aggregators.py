...
class Aggregator_2(nn.Module):
    def __init__(self, emb_dim):
        super(Aggregator_2, self).__init__()
    ...
    def forward(self, node):
        ...
        new_emb = self.update_embedding(torch.cat((curr_emb, node.data['identifier']), 1), nei_msg)
...
class MLPAggregator_2(Aggregator_2):
    def __init__(self, input_dim, output_dim):
        super(MLPAggregator_2, self).__init__(output_dim)
...
class GRUAggregator_2(Aggregator_2):
    def __init__(self, input_dim, output_dim):
        super(GRUAggregator_2, self).__init__(output_dim)
        self.gru = nn.GRUCell(input_dim, output_dim)

    def update_embedding(self, curr_emb, nei_msg):
        new_emb = self.gru(curr_emb, nei_msg)
