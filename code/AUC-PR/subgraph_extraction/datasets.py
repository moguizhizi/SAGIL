...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None,
                 ...
                 kge_model='', file_name='', init_add_identifiers=False, hit_eval=False,
    ...
    def __getitem__(self, index):
        ...
        for neg_rel in neg_rel_list:
            if neg_rel != r_label_pos:
                ...
                nodes_identifiers_from_subgraph_neg = []
                edge_identifiers_from_subgraph_neg = []
                identifiers_neg = []

                head_node_identifier = torch.zeros(len(self.max_nodes_identifiers)).tolist()
                tail_node_identifier = torch.zeros(len(self.max_nodes_identifiers)).tolist()
                if self.identifiers_from_all_graph is True:
                    identifiers_neg.append(head_node_identifier)
                    identifiers_neg.append(tail_node_identifier)
                else:
                    nodes_identifiers_from_subgraph_neg.append(head_node_identifier)
                    nodes_identifiers_from_subgraph_neg.append(tail_node_identifier)
                    neg_subgraph = dgl.DGLGraph(self.graph.subgraph(nodes_neg))
                    for _ in np.arange(neg_subgraph.number_of_edges()):
                        neg_edge_identifier = torch.zeros(len(self.max_edge_identifiers)).tolist()
                        edge_identifiers_from_subgraph_neg.append(neg_edge_identifier)

                subgraphs_neg.append(
                    self._prepare_subgraphs(nodes_neg, r_label_neg, n_labels_neg, identifiers=identifiers_neg,
                                            nodes_identifiers_from_subgraph=nodes_identifiers_from_subgraph_neg,
                                            edge_identifiers_from_subgraph=edge_identifiers_from_subgraph_neg,
                                            num_dim_edge_identifiers=self.num_dim_edge_identifiers))
