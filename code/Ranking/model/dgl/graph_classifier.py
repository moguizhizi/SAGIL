...
class GraphClassifier(nn.Module):
    ...
    def forward(self, data):
        ...
        norm_sparse = []
        for i in range(self.link_mode):
            norm_sparse.append(torch.sparse.mm(edge_connect_l[i], torch.ones(num_edges, 1).to(device=self.params.device)) + 1e-30)
        ...
        neg_relations = torch.tensor(neg_relations).to(device=self.params.device)
