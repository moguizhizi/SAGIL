...
def initialize_model(params, model, load_model=False):
    ...
    if load_model and os.path.exists(os.path.join(params.exp_dir, 'best_graph_classifier.pth')):
        ...
        graph_classifier = torch.load(os.path.join(params.exp_dir, 'best_graph_classifier.pth'), map_location=params.device)
        graph_classifier.params.device = params.device
    else:
...
def set_params_from_model(params, model):
    params.clamp_enable = model.params.clamp_enable
    params.cut_enable = model.params.cut_enable
    params.node_identifier_min = model.params.node_identifier_min
    params.node_identifier_max = model.params.node_identifier_max
    params.edge_identifier_min = model.params.edge_identifier_min
    params.edge_identifier_min = model.params.edge_identifier_max
    params.identifiers_from_all_graph = model.params.identifiers_from_all_graph
    params.version = model.params.version
    params.gsn_type = model.params.gsn_type
    params.one_hot_enable = model.params.one_hot_enable
    params.id_type = model.params.id_type
    params.k = model.params.k
    params.max_nodes_identifiers = model.params.max_nodes_identifiers
    params.max_edge_identifiers = model.params.max_edge_identifiers
    params.activate_func_type = model.params.activate_func_type
    params.edge_identifier_activate_func_type = model.params.edge_identifier_activate_func_type
    params.init_add_identifiers = model.params.init_add_identifiers
    params.gnn_agg_type = model.params.gnn_agg_type
    params.has_attn = model.params.has_attn
